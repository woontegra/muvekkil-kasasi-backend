...
export async function prepareManualWhatsApp(input: {
  ...
  req: Request
  /** false ise yalnızca iş kaydı oluşturur; wa.me açılmaz (Cloud API gönderimi için). */
  ...
  const shouldOpen = input.openDeepLink !== false
  if (existing) {
    if (!shouldOpen) {
      return { ok: true, status: 'DUPLICATE', jobId: existing.id, deepLinkUrl: null }
    }
